...
매 시간 정각에 알림음을 재생하고 데스크톱 알림을 표시하는 프로그램입니다.
...
5. **데스크톱 알림**: 정각마다 "HH시 정각입니다" 메시지 표시
...
다른 Mac 사용자에게 공유할 설치용 DMG 파일은 아래 명령으로 만듭니다.

```bash
./build_dmg.sh
```

빌드가 끝나면 `release/HourlyAlarm-1.0.0-macos-$(uname -m).dmg` 파일이 생성됩니다. DMG를 열면 `HourlyAlarm.app`과 `Applications` 바로가기가 보이고, 사용자는 `HourlyAlarm.app`을 `Applications`로 드래그해서 설치하면 됩니다.
